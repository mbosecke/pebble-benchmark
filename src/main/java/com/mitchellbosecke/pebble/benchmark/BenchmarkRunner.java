	...
	public void run() {
		...
		for (int trial = 1; trial <= this.trials; trial++) {
			for (Benchmark benchmark : benchmarks) {
				...
				double totalResultSoFar = results.containsKey(benchmark) ? results
						.get(benchmark) : 0;
				...
				System.out
						.println(String
								.format("Trial %d [%s]: There were %d reps, total time is %f seconds, average time per rep is %f seconds.",
										trial, benchmark.getClass()
												.getSimpleName(), reps, result,
										result / reps));
			}
		...
		System.out.println("===============================");
		System.out
				.println("Benchmarks completed. Average total time for each benchmark:");

		for (Entry<Benchmark, Double> result : this.results.entrySet()) {
			System.out.println(String.format("%s: %f", result.getKey()
					.getClass().getSimpleName(), result.getValue() / trials));
		}
